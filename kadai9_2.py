...
def check_acorr(i):
    result = acorr_ljungbox(nikkei_daily_df['price'].values, lags=i, return_df=True)
    print(result)
...
# train = nikkei_daily_df[0:365*3-2+30*5+1]  # 学習用データ 2022年5月31日まで
# test = nikkei_daily_df[365*3-2+30*5+1:]  # 検証用データ 2022年分

train = nikkei_daily_df[0:365*3-2+30*5+11]  # 学習用データ 2022年6月10日まで
test = nikkei_daily_df[365*3-2+30*5+11:]  # 検証用データ 2022年分

#print(nikkei_daily_df[0:849].tail(5))
print(nikkei_daily_df[0:365*3-2+30*5+11].tail(5))
...
def pred_nikkei_stock_average():
    ...
    SARIMA = sm.tsa.SARIMAX(train.price, order=(1, 1, 2), seasonal_order=(0, 0, 0, 0)).fit()
    pred = SARIMA.predict('2022-06-10', '2022-06-20')
    # 実データと予測結果の図示
